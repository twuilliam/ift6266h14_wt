...
from ift6266h14_wt.utils import load_data_w_phn, load_sentence
...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001,
   ...
   """
    ...
    # Load data
    datasets = load_data_w_phn(dataset[0], dataset[1])
...
#    sentence_x, sentence_y = datasets[3]
    ...
    while (epoch < n_epochs) and (done_looping is False):
        ...
        # print error
        logfile.write('epoch %i, train error %f, validation error %f, at %s' %
                      (epoch, this_train_loss, this_validation_loss, time.strftime("%Hh%Mm%Ss")))
        ...
        if epoch == n_epochs:
            # Saving the model set
            logfile.write('... saving last model')
            regression.save_model(filename='params_train.pkl', save_dir=output_folder)
    ...
    phonemes = load_sentence(dataset[2])
    num_pred = len(phonemes)
    y_gen = numpy.zeros(num_pred, dtype=theano.config.floatX)
    presamples = numpy.zeros((1, 240), dtype=theano.config.floatX)

    for i in xrange(num_pred):
        y_gen[i] = numpy.random.normal(
            ygen_model(numpy.concatenate((presamples, phonemes[i].reshape((1, 120))), axis=1)),
            numpy.sqrt(train_err[best_epoch-1]))
        presamples = numpy.roll(presamples, -1)
        presamples[-1] = y_gen[i]

    f = file(os.path.join(output_folder, 'output_raw.npy'), 'wb')
    numpy.save(f, y_gen)
    f.close()
    ...
    f = file(os.path.join(output_folder, 'output.npy'), 'wb')
    numpy.save(f, output)
    f.close()
...
def generate_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001,
    ...
    for i in xrange(num_pred):
        ...
        presamples = numpy.roll(presamples, -1)
        presamples[-1] = y_gen[i]
        print '%5f %5f %5f' % (i, y_gen[i], ygen_model(numpy.concatenate((presamples, phonemes[i].reshape((1, 120))), axis=1)))
    output = numpy.int16((y_gen+mean_timit)*std_timit)
