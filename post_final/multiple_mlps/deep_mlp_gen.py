...
class MLP(object):
    """Multi-Layer Perceptron Class
        ...
        """
    def __init__(self, rng, input, n_in, n_hidden, n_out, Winit=['0.pkl', '1.pkl']):
...
#    Winit = [os.path.join(output_folder, '0.pkl'), os.path.join(output_folder, '1.pkl')]

    # construct the MLP class
    regression = MLP(rng=rng, input=x, n_in=train_set_x.get_value(borrow=True).shape[1],
                     n_hidden=n_hidden, n_out=1, Winit=None)
...
def generate_mlp(n_hidden=500, output_folder='output', timelen=500,
                 previousdata='prev.npy', savedata='cur.npy'):
    ...
    # construct the MLP class
    regression = MLP(rng=rng, input=x, n_in=240,
    ...
    y_gen = numpy.zeros(timelen)
    presamples = numpy.load(previousdata, 'r')[-240:].astype(theano.config.floatX)
    for i in xrange(timelen):
...
if __name__ == '__main__':
    ...
    list_phn = numpy.load('list_phn.npy', 'r')
    length_phn = numpy.load('length_phn.npy', 'r')

    previous = None
    compteur = 1

    for i, j in zip(list_phn, length_phn):
        print i, j
        if previous is None:
            previous = 'timit_begin.npy'
            generate_mlp(n_hidden=n_hidden, output_folder='experiences/'+i, timelen=j,
                         previousdata=previous, savedata=str(compteur)+i+'.npy')
        else:
            generate_mlp(n_hidden=n_hidden, output_folder='experiences/'+i, timelen=j,
                         previousdata=str(compteur-1)+previous+'.npy', savedata=str(compteur)+i+'.npy')
        previous = i
        compteur += 1
